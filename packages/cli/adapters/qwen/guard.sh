...
if [ -n "$TOOL" ]; then
  printf '%s' "$INPUT" | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{try{const j=JSON.parse(s);process.stdout.write(JSON.stringify({event:'tool',tool:j.tool_name||j.tool,args:j.tool_input||j.input||{}}))}catch{process.stdout.write(JSON.stringify({event:'tool',tool:'',args:{}}))}})" | datacloak guard
else
